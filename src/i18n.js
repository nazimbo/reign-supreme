export const translations = {
  en: {
    score: 'Score',
    highScore: 'High Score',
    ruledFor: 'Ruled for',
    years: 'years',
    ruled: 'ruled',
    than: 'than',
    higher: 'Higher',
    lower: 'Lower',
    gameOver: 'Game Over!',
    yourScore: 'Your Score',
    playAgain: 'Play Again',
    highScoreShort: 'Best',
    menuSubtitle: 'Who ruled the longest?',
    menuHowTo: 'A leader is shown on the left with their reign duration. Guess whether the challenger on the right ruled for longer or shorter.',
    menuHowToMobile: 'A leader is shown on the top with their reign duration. Guess whether the challenger on the bottom ruled for longer or shorter.',
    play: 'Play',
    funFact: 'Fun Fact',
    leaderboard: 'Leaderboard',
    enterName: 'Enter your name',
    submitScore: 'Submit',
    rank: '#',
    player: 'Player',
    noScores: 'No scores yet',
    submitting: 'Submitting...',
    submitted: 'Submitted!',
    viewLeaderboard: 'View Leaderboard',
    backToMenu: 'Menu',
  },
  fr: {
    score: 'Score',
    highScore: 'Meilleur score',
    ruledFor: 'A régné',
    years: 'ans',
    ruled: 'a régné',
    than: 'que',
    higher: 'Plus long',
    lower: 'Moins long',
    gameOver: 'Perdu\u00a0!',
    yourScore: 'Votre score',
    playAgain: 'Rejouer',
    highScoreShort: 'Record',
    menuSubtitle: 'Qui a régné le plus longtemps\u00a0?',
    menuHowTo: 'Un dirigeant est affiché à gauche avec la durée de son règne. Devinez si le challenger à droite a régné plus longtemps ou moins longtemps.',
    menuHowToMobile: 'Un dirigeant est affiché en haut avec la durée de son règne. Devinez si le challenger en bas a régné plus longtemps ou moins longtemps.',
    play: 'Jouer',
    funFact: 'Le saviez-vous\u00a0?',
    leaderboard: 'Classement',
    enterName: 'Entrez votre pseudo',
    submitScore: 'Envoyer',
    rank: '#',
    player: 'Joueur',
    noScores: 'Aucun score',
    submitting: 'Envoi...',
    submitted: 'Envoyé\u00a0!',
    viewLeaderboard: 'Voir le classement',
    backToMenu: 'Menu',
  },
};
